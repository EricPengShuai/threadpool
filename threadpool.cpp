...
ThreadPool::~ThreadPool()
{
    ...
    //!NOTE: 这个顺序很重要，访问 notEmpty_ 所在线程先抢到锁 wait 之后无法被 notify 而死锁
    // 不过MacOS上似乎不会出现这个死锁
...
Result ThreadPool::submitTask(std::shared_ptr<Task> sp)
{
    ...
    // 创建新线程的条件：#1 任务数量多于 空闲线程; #2 当前线程数量小于上限阈值
    if (poolMode_ == PoolMode::MODE_CACHED && 
        ...
        curThreadSize_ < threadSizeThreshHold_)
    {
        ...
        // auto ptr = std::make_unique<Thread>(std::bind(&ThreadPool::threadFunc_v0, this, std::placeholders::_1));
        // threads_.emplace_back(std::move(ptr));
        int threadId = ptr->getId();
        threads_.emplace(threadId, std::move(ptr));
        std::cout << ">>> create new thread... id = " << threadId << std::endl;
...
void ThreadPool::start(int initThreadSize)
{
    ...
    for (int i = 0; i < initThreadSize_; ++ i) 
    {
        ...
        // auto ptr = std::make_unique<Thread>(Thread(std::bind(&ThreadPool::threadFunc_v0, this, std::placeholders::_1)));
        // threads_.emplace_back(std::move(ptr));
...
// 初始版本的线程函数，不等待线程执行任务结束
void ThreadPool::threadFunc_v0(int threadId)
{
    auto lastTime = std::chrono::high_resolution_clock().now();
    ...
    while (isPoolRunning_) 
    {
        std::shared_ptr<Task> task;
        {
            ...
            while (isPoolRunning_ && taskQueue_.size() == 0)
            {
                ...
                // if (!isPoolRunning_) {
                //     threads_.erase(threadId);
                //     std::cout << "~threadId: " << threadId << ' ' << std::this_thread::get_id() << " 111exit!\n";
                //     exitCond_.notify_all();
                //     return ;
                // }
            }
    ...
    threads_.erase(threadId);
    std::cout << "~threadId: " << threadId << ' ' << std::this_thread::get_id() << " 222exit!\n";
    exitCond_.notify_all();
}
